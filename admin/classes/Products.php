...
class Products
{
	...
	public function getProducts(){
		...
		if($uname != 'karwan@gmail.com'){
			...
			$q = $this->con->query("SELECT p.product_id, p.product_title, p.product_price,p.product_qty, p.product_desc, p.product_image, c.cat_title, c.cat_id, b.brand_id, b.brand_title FROM products p JOIN categories c ON c.cat_id = p.product_cat JOIN brands b ON b.brand_id = p.product_brand WHERE p.admin_name= '$uname'");
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."../images/".$uniqueImageName)) {

					$uname= $_SESSION['user'];
					
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`, `admin_name`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName', '$uname')");
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			...
			$cn= explode(" ",$name);
			$c_name= implode("_", $cn);
			$q = $this->con->query("INSERT INTO categories (cat_title,admin_name,cat_name) VALUES ('$name','$uname','$c_name')");
			if ($q) {
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			...
			$q = $this->con->query("INSERT INTO brands (brand_title,admin_name) VALUES ('$name', '$uname')");
			if ($q) {
